...
class Wav2vec_UConfig(FairseqDataclass):
    ...
    generator_type: str = "cnn"  # choices={"perfect", "cnn", "average"}
    gan_type: str = "js_div"  # choices={"kl_div", "js_div", "l1", "vanilla", "mmd", "wasserstein"}
    generator_path: str = ""
    switch_frequency: int = 1
...
class MLPDiscriminator(nn.Module):
    ...
    def forward(self, x, padding_mask, reduction="sum"):
        #x = self.net(x).squeeze(-1)
        x = torch.stack(
            [layer(x[:, i]) for i, layer in enumerate(self.layers)], 
            dim=1,
        ).squeeze(-1)
        if reduction:
            x = x.sum(-1)
...
class Wav2vec_U(BaseFairseqModel):
    ...
    def discrim_step(self, num_updates):
        return num_updates % self.switch_freq < (self.switch_freq - 1)

    def reset_step(self, num_updates):
        return num_updates % self.switch_freq == 0
    ...
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        self.switch_freq = cfg.switch_frequency 
        self.reset_discriminator_every_update = cfg.reset_discriminator_every_update

        if self.discriminator_type == "linear":
            self.discriminator = LinearDiscriminator(output_size, cfg)
        ...
        if self.generator_type == "perfect":
            with open(cfg.generator_path, "r") as f:
                lines = f.read().strip().split("\n")
                weight = torch.eye(output_size)[
                    [int(i)+self.nspecial for i in lines]
                ].t().unsqueeze(-1)
                with torch.no_grad():
                    for p in self.generator.parameters():
                        if p.ndim == weight.ndim:
                            p.copy_(weight)
    ...
    def forward(
        ...
        if not (self.no_softmax and dense_x_only):
            if self.no_softmax:
                ...
                if self.no_silence:
                    dense_x[..., self.zero_index] = 0.0
            else:
                if self.no_special_tokens:
                    dense_x[..., :self.nspecial] = -1e14
                if self.no_silence:
        ...
        if d_step:
            if self.reset_discriminator_every_update and self.reset_step(self.update_num):
        ...
        if self.generator_type == "average":
            ...
            if self.discriminator_type == "perfect":
                ...
                avg_dense_and_token_logits = torch.maximum(
                    ...
                    1e-10*torch.ones(
                        avg_dense_and_token_x.size(), 
                        device=avg_dense_and_token_x.device,
                    ),
                ).log()
                loss_d = F.kl_div(
                    avg_dense_and_token_logits,
                    avg_token_x,
                    reduction="sum",
                ) + F.kl_div(
                    avg_dense_and_token_logits,
                ...
                if self.gan_type == "kl_div":
                    loss = F.kl_div(
                        torch.log_softmax(avg_dense_logits, dim=-1), 
                        avg_token_x,
                        reduction="batchmean",
                    )
                ...
                elif self.gan_type == "js_div":           
                    loss = loss_d
                else:
                    raise ValueError(
                        "Unknown GAN type: {self.gan_type}"
                    )
                result = {
                    "losses": {
                        "dense_g": loss,
                        ...
                        "dense_d": loss_d,
                        "token_d": loss_d,
                        "grad_pen": torch.tensor(0.0),
                        "code_pen": torch.tensor(0.0),
                        "smoothness": torch.tensor(0.0),
                    },
                    "temp": 0,
                    "code_ppl": torch.tensor(0.0),
                    "prob_ppl": torch.tensor(0.0),
                    "d_steps": 0,
...
#                    avg_dense_x = avg_dense_logits
                ...
                sample_size = 1
        elif self.generator_type == "unigram_average":
            bsz, tsz, dsz = dense_x.size()
            cost_input = torch.eye(
                dsz, device=dense_x.device,
            ).expand(bsz, tsz, -1, -1) 
            cost_input = cost_input.permute(0, 2, 1, 3).reshape(
                -1, tsz, dsz,
            )
            costs = self.discriminator(cost_input, None, None)
            costs = costs.reshape(bsz, tsz, dsz)
            if self.gan_type == "least_square":
                if d_step:
                    dense_y = (dense_x * (costs + 1)**2).sum((-2, -1))
                    token_y = (token_x * (costs - 1)**2).sum((-2, -1))
                else:
                    dense_y = (dense_x * (costs**2)).sum((-2, -1))
            elif self.gan_type == "vanilla":
                if d_step:
                    dense_y = (-dense_x * F.logsigmoid(costs)).sum((-2, -1))
                    token_y = (-token_x * F.logsigmoid(-costs)).sum((-2, -1))
                else:
                    dense_y = (-dense_x * F.logsigmoid(-costs)).sum((-2, -1))
            elif self.gan_type == "mmd":
                if d_step:
                    dense_y = (dense_x * costs).sum((-2, -1))
                    token_y = - (token_x * costs).sum((-2, -1))
                else:
                    dense_y = (token_x * costs).sum((-2, -1)) - (dense_x * costs).sum((-2, -1))
            else:
                raise ValueError(f"Not supporting unigram average generator for {self.gan_type} GAN")
            sample_size = features.size(0)
        else:
        ...
        if d_step:
            if self.generator_type == "unigram_average":
                ...
                if self.gan_type in ["mmd", "wasserstein"]:
                    loss_dense = torch.mean(dense_y) * sample_size
                    loss_token = - torch.mean(token_y) * sample_size
                elif self.gan_type == "least_square":
                    loss_dense = torch.mean((dense_y + 1)**2) * sample_size
                    loss_token = torch.mean((token_y - 1)**2) * sample_size
                else:
            ...
            if self.generator_type == "unigram_average":
                ...
                if self.gan_type in ["mmd", "wasserstein"]:
                    ...
                    loss_dense = torch.mean(dense_y**2) * sample_size
                else:
                    loss_dense = F.binary_cross_entropy_with_logits(
                        dense_y,
